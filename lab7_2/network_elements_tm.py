from math import log10, floor, exp, pi
...
class Lightpath(Signal_information):
    def __init__(self, signal_power_value=1e-3, given_path=None, selected_channel=1):
        ...
        self.__rs: float = 32.0 # symbol rate, GHz
        self.__df: float = 50.0 # frequency spacing, GHz
    ...
    def get_frequency_spacing(self) -> float:
        return self.__df
    ...
    def set_symbol_rate(self, new_rate: float):
        self.__rs = new_rate

    def set_frequency_spacing(self, new_spacing: float):
        self.__df = new_spacing
...
class Node:
    ...
    def propagate(self, signal: Lightpath):
        ...
        if len(signal_path) > 1:
            ...
            signal.set_signal_power(        # 3. set optimal launch power for each line
                self.__successive[successive_line].optimized_launch_power(signal)
            )
...
class Line:
    def __init__(self, label: str = "default", length: float = 0.0):
        ...
        self.__amp_gain_db: float = 16.0
        self.__amp_nf_db: float = 5.5
        self.__nli_params: dict = {
            "a_db": 0.2,    # dB/km
            "a_lin": 0.2/(20.0*log10(exp(1))),
            "beta2": 2.13,   # e-26, ps2 / km
            "gamma": 1.27e-3    # 1 / W m
        }
    ...
    @staticmethod
    def to_db(lin_value: float) -> float:
        return 10.0*log10(lin_value)
    def latency_generation(self, signal: Signal_information):
        added_latency = self.__length/2e08
        signal.update_latency(added_latency)

    def noise_generation(self, signal: Lightpath):  # 1. edited formula
        # generated_noise = 1e-9 * signal.get_signal_power() * self.__length
        generated_noise = self.ase_generation() + self.nli_generation(signal)
        signal.update_noise_power(generated_noise)

    def ase_generation(self) -> float: #    returns in LINEAR UNITS
        h = 6.62607015  # e-34, m2 kg/s
        f = 193.414     # THz, so e+12
        b_n = 12.5      # GHz, so e+9
        nf = self.to_lin(self.__amp_nf_db)
        g = self.to_lin(self.__amp_gain_db)
        return self.__n_amplifiers * (
            h * f * b_n * nf * (g-1)
        ) * 1e-13   # 10^(-34 + 12 + 9)

    def evaluate_n_nli(self, signal: Lightpath) -> float:
        a_lin = self.__nli_params["a_lin"]/1000     # 1/m
        l_eff = 1.0/(2*a_lin)                       # m
        beta2 = self.__nli_params["beta2"]          # e-26
        rs = signal.get_symbol_rate()               # e+9
        df = signal.get_frequency_spacing()         # e+9
        gamma = self.__nli_params["gamma"]
        nch = self.__channels

        return (16.0/(27.0 * pi))*log10(
            ((pi**2)/2.0)*((beta2*(rs**2)*1e-8)/a_lin)*(nch**(2*rs/df))
        ) * (a_lin*(gamma**2)*(l_eff**2))/(beta2*(rs**3)*10)

    def nli_generation(self, signal: Lightpath) -> float:
        p_ch = signal.get_signal_power()
        n_nli = self.evaluate_n_nli(signal)
        n_span = self.__n_amplifiers + 1
        b_n = 12.5e+9   # noise bandwidth
        return (p_ch**3)*n_nli*n_span*b_n

    def optimized_launch_power(self, signal: Lightpath) -> float:   # 2. formula to have optimized launch power
        p_ase = self.ase_generation()
        n_span = self.__n_amplifiers + 1
        b_n = 12.5e+9  # noise bandwidth
        n = self.evaluate_n_nli(signal)

        return (p_ase / (2*n*b_n*n_span)) ** (1.0/3.0)
    ...
    def set_n_amplifiers(self, new_amplifiers):
        try:
            self.__n_amplifiers = int(new_amplifiers)
        except ValueError:
            print("The value given to the set_n_amplifiers method was not an integer number.")
    ...
    def __str__(self):
        message = dedent(f"""\
                        ...
                        Successive:         {", ".join(self.__successive.keys())}
                        # of amplifiers:    {self.__n_amplifiers}\n"""
                         )
        return message
