...
class Lightpath(Signal_information):
    def __init__(self, signal_power_value=1e-3, given_path=None, selected_channel=1):
        ...
        self.__rs: float = 32e+9    # signal symbol rate
        self.__df: float = 50e+9    # frequency spacing between consecutive channels
...
class Line:
    ...
    def ase_generation(self):
        f = 193.414e+12                             # frequency fixed at the C-band center
        bn = 12.5e+9                                # noise bandwidth
        nf_l = 10**(self.__noise_figure_db/10.0)    # noise figure of amplifiers in linear units
        g_l = 10**(self.__gain_db/10.0)             # gain of amplifiers in linear units
        return self.__n_amplifiers * (
            spc.h * f * bn * nf_l * (g_l - 1)       # spc.h is the Planck constant
        )

    def nli_generation(self, signal: Lightpath, n_channels):
        bn = 12.5e+9
        n_nli = (16.0/(27.0*spc.pi)) * \
            log10(
                (spc.pi ** 2) * self.__beta * (signal.get_rs() ** 2) *
                (n_channels ** (2.0*signal.get_rs()/signal.get_df())) / (2.0 * self.__alpha)
            ) * \
            self.__alpha * (self.__gamma ** 2) * (self.__effective_length ** 2) / \
            (self.__beta * ((signal.get_rs()) ** 3))
