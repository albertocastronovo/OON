...
class Network:
    def __init__(self, file_path: str = None, channels: int = 10):
        ...
        self.__utm_pairs: list[tuple[str, str]] = []
    ...
    def network_analysis(self):
        ...
        for path in self.__all_paths:
            ...
            # print("signal power: " + str(test_signal.get_signal_power()))
            # print("noise power: " + str(test_signal.get_noise_power()))
            ...
            paths_for_rs.append(
                ["->".join(path)] + [str(self.__channels)] + ["1" for _ in range(self.__channels)]
            )
    ...
    def stream(self, connection_list: list[Connection], best="latency"):
        ...
        for connection in connection_list:
            ...
            try:
                ...
                strategy = "fixed_rate"
            lighpath_for_br = Lightpath(
                signal_power_value=1e-3,
                given_path=path.copy(),
                selected_channel=1
            )
            bit_rate = self.calculate_bit_rate(signal=lighpath_for_br, strategy=strategy)
            ...
            if path[0] == "empty" or bit_rate is None or bit_rate < 0.1:
                ...
                allocated_bit_rates.append(0)
                self.__capacity["rejected"] += 1
            else:
                success_count += 1
                self.__capacity["accepted"] += 1
                ...
                allocated_bit_rates.append(bit_rate)
                self.routing_space_update()
        # print("total connections: " + str(total_count))
        # print("total failures: " + str(fail_count))
        self.restore_all_matrices()
        return allocated_bit_rates

    def stream_random_from_tm(self, traffic_matrix: dict[str: dict[str: float]], number_of_connections=1):
        if len(self.__utm_pairs) == 0:
            self.__utm_pairs = [(i, j) for i in self.get_node_labels() for j in self.get_node_labels() if j != i]
            if len(self.__utm_pairs) == 0:
                print("No pairs left!")
                return 0
        bit_rates = []
        for _ in range(number_of_connections):
            ...
            while True:
                try:
                    nodes = choice(self.__utm_pairs)
                except IndexError:
                    return bit_rates
                if traffic_matrix[nodes[0]][nodes[1]] > 0:
                    break
                else:
                    self.__utm_pairs.remove((nodes[0], nodes[1]))
                watchdog += 1
                if watchdog > 50:
                    raise Exception("Random stream timeout")
            connection = [Connection(list(nodes), 1e-3)]
            bit_rate = self.stream(connection, best="snr")
            traffic_matrix[nodes[0]][nodes[1]] -= bit_rate[0]
            if traffic_matrix[nodes[0]][nodes[1]] < 0:
                self.__utm_pairs.remove((nodes[0], nodes[1]))
            bit_rates.append(bit_rate[0])
        return bit_rates
    ...
    def calculate_bit_rate(self, signal: Lightpath, strategy: str = "fixed_rate"):
        ...
        try:
            ...
            return None
        # print("gsnr: " + str(gsnr))
        r_s = signal.get_rs()           # Rs = symbol rate, fixed to 32 (GHz)
