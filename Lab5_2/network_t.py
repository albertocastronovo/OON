...
from network_elements_t import Node, Line, Connection, Lightpath
...
class Network:
    ...
    def calculate_bit_rate(self, path: str, strategy: str) -> float:
        ber_t = 1e-3    # bit error rate
        r_s = 32.0      # symbol rate of light path, in GHz
    ...
    def stream(self, connection_list: list[Connection], best="latency") -> dict:
        ...
        for connection in connection_list:
            ...
            if path[0] == "empty":
                ...
                bit_rate = self.calculate_bit_rate("->".join(path), strategy)
                if bit_rate < 1:
