...
class Network:
    def __init__(self, file_path: str = None, channels: int = 10):
        ...
        self.__rsu_Vectorized_complete = np.vectorize(self.__rsu_to_vectorize_complete)
    ...
    def connect(self):
        ...
        for line in self.__lines.values():  # set successive attribute of each line
            line.set_channels(self.__channels)
            line.set_successive(
                {
    ...
    def calculate_bit_rate(self, lightpath: Lightpath, strategy: str) -> float:    #   4. modified to get a lightpath
        path = "->".join(lightpath.get_path())
        ber_t = 1e-3                        # bit error rate
        r_s = lightpath.get_symbol_rate()   # symbol rate of light path, in GHz
        b_n = 12.5                          # noise bandwidth, in GHz

        gsnr_cell = self.__weighted_paths.loc[self.__weighted_paths["Path"] == path, "SNR [dB]"]
        gsnr = gsnr_cell.iloc[0]
        if strategy == "fixed-rate":
            if gsnr >= self.to_db(2.0*(erfcinv(2.0*ber_t)**2)*r_s/b_n):
                br = 100.0
            else:
                br = 0.0
        elif strategy == "flex-rate":
            if gsnr >= self.to_db(10.0*(erfcinv(8.0*ber_t/3.0)**2)*r_s/b_n):
                br = 400
            elif gsnr >= self.to_db((14.0/3.0)*(erfcinv(8.0*ber_t/3.0)**2)*r_s/b_n):
                br = 200
            elif gsnr >= self.to_db(2.0*(erfcinv(2.0*ber_t)**2)*r_s/b_n):
                br = 100
            else:
                br = 0.0
        elif strategy == "shannon":
            gsnr_lin = self.to_lin(gsnr)
            br = 2*r_s*log2(1 + gsnr_lin*r_s/b_n)
        else:
            br = -1.0
        return br       # in Gbps
    ...
    @staticmethod
    def to_lin(db_value: float) -> float:
    ...
    def reset_all_lines(self):
        for line in self.__lines.keys():
            self.__lines[line].set_state([1 for _ in range(self.__channels)])
    ...
    def stream_random_from_tm(self, traffic_matrix: dict[dict[float]]) -> (int, Connection):
        filtered_rows_matrix = {key: val for key, val in traffic_matrix.items() if sum(val.values()) > 0.1}
        if not filtered_rows_matrix:
            return -2, Connection(["error", "error"])
        selected_row = choice(list(filtered_rows_matrix.keys()))
        filtered_col = {key: val for key, val in traffic_matrix[selected_row].items() if val > 0.1}
        selected_col = choice(list(filtered_col.keys()))
        requested_con = Connection([selected_row, selected_col])
        creq = self.stream([requested_con], best="snr")
        if creq["allocated"] > 0:   # the request was accepted
            traffic_matrix[selected_row][selected_col] = max(
                0.0,
                float(traffic_matrix[selected_row][selected_col] - requested_con.get_bit_rate())
            )
            return 0, requested_con
        else:
            # print("connection refused")
            return -1, requested_con


    def stream(self, connection_list: list[Connection], best="latency") -> dict:
        ...
        for connection in connection_list:
            if best == "latency":
                ...
                return {"return": -1}
            if path[0] == "empty":
                # print("generic rejection!")
                connection.set_latency(-1)
                connection.set_snr(0.0)
                connection.set_bit_rate(0.0)
            else:
                strategy = self.__nodes[path[0]].get_transceiver()
                lp_for_br = Lightpath(given_path=path.copy())       #   4. edit to pass a lightpath to bit rate calc
                bit_rate = self.calculate_bit_rate(lp_for_br, strategy)
                if bit_rate < 1:
                    print("rejection due to bit rate!")
                    connection.set_latency(-1)
                    connection.set_snr(0.0)
                    connection.set_bit_rate(0.0)
                    continue
                ...
                # print("stato successivo: " + self.__route_space.loc[self.__route_space.Path == "->".join(old_path)].to_string())
                connection.set_latency(test_signal.get_latency())
                connection.set_snr(test_signal.get_snr())
        self.reset_all_nodes_sm()
        return {
            "return": 0,
            "allocated": float(success)/float(all_connections),
            "two-nodes": two_nodes_paths,
            "three-nodes": three_nodes_paths
        }

    def reset_all_nodes_sm(self):
        for node in self.__nodes.values():
            node.reset_switching_matrix()
    ...
    def __rsu_to_vectorize_complete(self, path):
        final_value_list = []
        path_list = path.split("->")
        lines = self.lines_in_path(path_list)
        for channel in range(1, self.__channels + 1):
            final_value = 1
            for line in lines:
                final_value *= self.__lines[line].get_channel_state(channel)
            if len(path_list) > 2:
        ...
        return tuple(final_value_list)

    def routing_space_update(self):

        mode = "base"

        if mode == "base":          # 101.34 seconds
            for i in range(1, self.__channels + 1):
        ...
        elif mode == "alternate":   # 273.67 seconds
            cols = [f"CH_{i}" for i in range(1, self.__channels + 1)]
            self.__route_space[cols] = self.__route_space.apply(
                lambda x: self.__rsu_Vectorized_complete(x["Path"]), axis=1, result_type="expand"
            )
